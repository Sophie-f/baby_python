def create_grades_dict(filename):
    ...
    my_file.close()
    grade_list = [0 for k in range(numbern_test)]
    grade_dict = {}
    for line in my_list:
...
def print_grades(file_name):
    ...
    for key in key_list:
        print("{0:10s} | {1:16s} | {2:6d} | {3:6d} | {4:6d} | {5:6d} | {6:6.2f} |".format(key,grades_dict[key][0],grades_dict[key][1],grades_dict[key][2],grades_dict[key][3],grades_dict[key][4],grades_dict[key][5]))
